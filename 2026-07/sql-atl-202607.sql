...
where  json_value ( sere.sensor_data, '$.sensorId' returning number ) = 1
and    json_value ( sere.sensor_data, 
         '$.metrics[*]?(@.metricType == "temperature").reading' returning number ) > 38;


/* 
  There will be thousands of rows/sensor
  JSON can be large
  => sensorId is a reasonable partition key
*/




/* Partition the table by sensor */
alter table sensor_readings
  modify nonpartitioned ;
  partition by hash (
    /* Partition by expression: 23.26.1 */
    json_value ( sensor_data, '$.sensorId' returning number ) 
  )
  partitions 8;
...
/* Queries by sensor access one partition */
select * from sensor_readings sere
where  json_value ( sere.sensor_data, '$.sensorId' returning number ) = 1;

select * from sensor_readings sere
where  json_value ( sere.sensor_data, '$.sensorId' returning number ) = 1
and    json_value ( sere.sensor_data, '$.metrics[*]?(@.metricType == "temperature").reading' returning number ) < 5;
...
where  json_value ( sere.sensor_data, '$.sensorId' returning number ) = 1
and    recorded_at >= trunc ( systimestamp - interval '1' day )
and    recorded_at < trunc ( systimestamp );




/* Still need to query sensorId directly for partition elimination */
select count(*)
from   sensor_readings sere
cross  join json_table ( 
          sere.sensor_data, '$' 
...
/* Now any JSON queries can use partition elimination and index */
select jt.* from sensor_readings sere
cross  join json_table (
  sere.sensor_data, '$.metrics[*]'
  columns (
...
drop index weather_sensor_json_i;
/* Recreate search index without sensorId and recordedAt */
create search index weather_sensor_json_i 
  on sensor_readings sere ( sere.sensor_data )
  for json
  -- 23.6 Exclude sensorId and recordedAt from index 
  parameters (
    'SEARCH_ON 
     TEXT EXCLUDE ( $.sensorId, $.recordedAt )'
  );
  -- local;
/* Alternative: include only the fields you want indexed */
  -- parameters ( 'SEARCH_ON TEXT INCLUDE ( $.metrics.metricType, $.metrics.reading )' )

/* But how do we index the temperature query?! */


/* What about other time units? */
...
/* e.g. last week's reading? */
select * from sensor_readings sere
where  json_value ( sere.sensor_data, '$.sensorId' returning number ) = 1
and    trunc ( sere.sensor_data.recordedAt.timestamp(), 'iw' ) = 
         trunc ( sysdate - 7, 'iw' );

/* Or last quarter's */
select * from sensor_readings sere
where  json_value ( sere.sensor_data, '$.sensorId' returning number ) = 1
and    trunc ( sere.sensor_data.recordedAt.timestamp(), 'q' ) = 
         trunc ( add_months ( sysdate, -3 ), 'q' );



/* Now change that to the previous fiscal quarter...  :/ */
/* Switching time units is hard! */
